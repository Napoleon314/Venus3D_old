...
bool WindowsMouse::_CreateCursor(VeCursor::Data* pkCur,
	...
	int32_t i32HotX, int32_t i32HotY) noexcept
{
	assert(spSurface && pkCur);
	const size_t pad = (sizeof(size_t) * 8);
	HICON hicon;
	HDC hdc;
	BITMAPV4HEADER bmh;
	LPVOID pixels;
	...
	int32_t w = spSurface->GetWidth();
	int32_t h = spSurface->GetHeight();
	...
	maskbits = VeStackAlloc(uint8_t, maskbitslen);
	...
	assert(spSurface->GetFormat()->m_u32Format == VE_PIXELFORMAT_ARGB8888);
	assert(spSurface->GetPitch() == w * 4);
	memcpy(pixels, spSurface->GetBuffer(), h * spSurface->GetPitch());

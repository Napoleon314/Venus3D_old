...
class build_info:
	def __init__(self, compiler, archs, cfg):
		try:
			...
			build_cfg.compiler = "auto"
		try:
			build_cfg.toolset
		except:
			build_cfg.toolset = "auto"
		try:
			build_cfg.arch
		except:
			build_cfg.arch = ("x86", "x64")
		try:
			build_cfg.config
		except:
			build_cfg.config = ("Debug", "Release", "MinSizeRel", "RelWithDebInfo")
		try:
			build_cfg.target
		except:
			build_cfg.target = "auto"
		...
		if "auto" == build_cfg.target:
			target_platform = host_platform
		else:
			target_platform = build_cfg.target
		...
		if "" == compiler:
			if ("" == build_cfg.compiler) or ("auto" == build_cfg.compiler):
...
def build_project(name, build_path, build_info, script_path, compiler_info, need_build = False, need_install = False, additional_options = ""):
	...
	additional_options += " -DBUILD_ARCH_NAME:STRING=\"%s\"" % compiler_info.arch

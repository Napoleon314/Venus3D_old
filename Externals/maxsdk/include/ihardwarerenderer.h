...
public:
	...
	\param[in] clearBlock  - this will also remove any material list that are present.  Typically you do not want to do this so on the next add the data struct will not need to be rebukt
	*/
	...
	\param[in] node this is the inode that owns this d3d mesh, it can be null
	\param[in] pMesh  this is the handle of the directx mesh which is a pointer to a D3DXMesh
	\param[in] mtl this is the material for this mesh and can be null
	\param[in] pHWMat this is the hwmaterial for this mesh and must be supplied
	\param[in] wireframeColor this is color of the wireframe mesh since it can be different than the shaded diffuse color
	\param[in] tmOverRide this is the world space transform for the mesh, if null the gw transform will be used

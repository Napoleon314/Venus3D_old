...
// this function is used by return_value macro to place the value in the 'current_results' thread local
// and pop the current local frame. return_value is implemented this way to ensure that code that
// looks like return_value(new Point3()) works correctly. Up through 3ds Max 2012, 2 new values ended
// up being created
template<typename Type> Type templated_return_value_fn(Type r, Value** link)
{
	thread_local(current_result) = r;
	thread_local(current_locals_frame) = link;
	return r;
}

// use this macro to return a value that is protected from garbage collection
// from a function using value_locals (pops value local storage)
#define return_value(r)	{ return templated_return_value_fn(r, vl.link); }
	 
// this function is used by return_value_no_pop and return_protected macros to place the value in the 
// 'current_results' thread local. These macros are implemented this way to ensure that code that
// looks like return_protected(new Point3()) works correctly. Up through 3ds Max 2012, 2 new values
// ended up being created.
template<typename Type> Type templated_return_protected_fn(Type r)
{
	thread_local(current_result) = r;
	return r;
}

// use these macros to return a value that is protected from garbage collection
// from a function not using value_locals (does not pop value local storage)
